...
public class Recipes extends RecipeEntry {
  ...
  public Recipes(){
    ...
    RecipeParsers.registerConsumeParser(ConsumeType.item, (b, r, c, h) -> {
      if (c instanceof ConsumeItemCond<?>){
        for (ItemStack stack : c.consItems) {
          h.get(r.addMaterial(itemsManager.getItem(stack.item), stack.amount)
              .setAttribute(c)
              .setMaxAttr());
        }
      }
    ...
    RecipeParsers.registerProduceParser(ProduceType.item, (b, r, p, h) -> {
      if (p.random){
        ...
        for (ItemStack item : p.items) {
          h.get(r.addProduction(itemsManager.getItem(item.item), 1).setFormat(f -> Mathf.round(item.amount/l*100) + "%"));
        }
      }
      else {
        for (ItemStack stack : p.items) {
          h.get(r.addProduction(itemsManager.getItem(stack.item), stack.amount));
        }
      }
    });
    ...
    RecipeParsers.registerConsumeParser(ConsumeType.liquid, (b, r, c, h) -> {
      if (c instanceof ConsumeLiquidCond<?>){
        for (LiquidStack stack : c.consLiquids) {
          h.get(r.addMaterial(itemsManager.getItem(stack.liquid), stack.amount)
              .setAttribute(c)
              .setMaxAttr());
        }
      }
        ...
        for (LiquidStack stack : c.consLiquids) {
          h.get(r.addMaterialPresec(itemsManager.getItem(stack.liquid), stack.amount));
        }
      }
    });
    RecipeParsers.registerProduceParser(ProduceType.liquid, (b, r, p, h) -> {
      if (p.portion){
        for (LiquidStack liquid : p.liquids) {
          h.get(r.addProduction(itemsManager.getItem(liquid.liquid), liquid.amount));
        }
      }
      for (LiquidStack liquid : p.liquids) {
        h.get(r.addProductionPresec(itemsManager.getItem(liquid.liquid), liquid.amount));
      }
    });

    //power
    RecipeParsers.registerConsumeParser(ConsumeType.power, (b, r, c, h) -> {
      h.get(r.addMaterialPresec(PowerMark.INSTANCE, c.usage));
    });
    RecipeParsers.registerProduceParser(ProduceType.power, (b, r, p, h) -> {
      h.get(r.addProductionPresec(PowerMark.INSTANCE, p.powerProduction));
    });

    //neutron energy
    RecipeParsers.registerConsumeParser(SglConsumeType.energy, (b, r, c, h) -> {
      h.get(r.addMaterial(EnergyMarker.INSTANCE, c.usage)
          .setFormat(f -> (f * 60.0F > 1000.0F ? UI.formatAmount((long)(f * 60.0F)) : Strings.autoFixed(f * 60.0F, 2)) + SglStatUnit.neutronFluxSecond.localized()));
    });
    RecipeParsers.registerProduceParser(SglProduceType.energy, (b, r, p, h) -> {
      h.get(r.addProduction(EnergyMarker.INSTANCE, p.product)
          .setFormat(f -> (f * 60.0F > 1000.0F ? UI.formatAmount((long)(f * 60.0F)) : Strings.autoFixed(f * 60.0F, 2)) + SglStatUnit.neutronFluxSecond.localized()));
    });
    ...
    RecipeParsers.registerConsumeParser(ConsumeType.payload, (b, r, c, h) -> {
      for (PayloadStack stack : c.payloads) {
        if (stack.amount > 1) h.get(r.addMaterial(itemsManager.getItem(stack.item), stack.amount));
        else h.get(r.addMaterial(itemsManager.getItem(stack.item), stack.amount).setFormat(f -> ""));
      }
    });
    RecipeParsers.registerProduceParser(ProduceType.payload, (b, r, p, h) -> {
      for (PayloadStack stack : p.payloads) {
        if (stack.amount > 1) h.get(r.addProduction(itemsManager.getItem(stack.item), stack.amount));
        else h.get(r.addProduction(itemsManager.getItem(stack.item), stack.amount).setFormat(f -> ""));
      }
    });
    ...
    RecipeParsers.registerConsumeParser(SglConsumeType.floor, (b, r, c, h) -> {
      for (ObjectFloatMap.Entry<Floor> entry : c.floorEff) {
        ...
        h.get(r.addMaterial(itemsManager.getItem(entry.key), b.size*b.size)
            .setOptionalCons(c.baseEfficiency > 0)
            .setEfficiency(eff)
            .setAttribute(c)
            .setFormat(f -> "[#98ffa9]" + (c.baseEfficiency > 0.001f? "+": "") + Mathf.round(eff*100) + "%"));
      }
    });
  }
  ...
  @Override
  public void init() {
    ((GeneratorRecipe) RecipeType.generator).powers.add(EnergyMarker.INSTANCE);
